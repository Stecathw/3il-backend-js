...
describe('Test the root path', () => {
  ...
  beforeAll(async () => {
    ...
    await prismaRepository.seed(); // Inserting Male/Female/Other and Two users Genre
  });
  ...
  test('It should return the welcome message', async () => {
    ...
    expect(response.text).toBe(
      'Welcome on my contacts API, all routes are founbd under <base_url>/api/contacts/'
    );
  });
  ...
  test('Should delete user', async () => {
    ...
    expect(res.status).toEqual(200);
    expect(res.text).toEqual(`Contact with ID ${id} deleted successfully`);
  });
